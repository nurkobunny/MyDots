...
gruvbox_wallpapers="$wallpaper_base_path/GruvBox"
tokyonight_wallpapers="$wallpaper_base_path/TokyoNight"
...
if [ "$(cat $HOME/.cache/.theme_mode)" = "TokyoNight" ]; then
    next_mode="GruvBox"
    # Logic for Dark mode
    wallpaper_path="$gruvbox_wallpapers"
else
    next_mode="TokyoNight"
    # Logic for Light mode
    wallpaper_path="$tokyonight_wallpapers"
fi
