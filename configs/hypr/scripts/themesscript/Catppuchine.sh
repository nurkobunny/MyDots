...
sed -i 's/^.*col\.active_border.*/col.active_border = rgb(89b4fa)/' "$USER_DECOR"
sed -i 's/^.*col\.inactive_border.*/col.inactive_border = rgb(6e738d)/' "$USER_DECOR"
sleep 1

### 🖼️ Случайные обои Gruvbox через swww
WALL_DIR="$HOME/Pictures/wallpapers/Dynamic-Wallpapers/Catppuchine/"
...
#Hyprlock
HYPRLOCK_CONFIG="$HOME/.config/hypr/hyprlockthemes/Catppuchine.conf"
...
#FASTFETCH
WALLALLDIREC=$HOME"/Pictures/fastfetch/catppuchine"
...
GRADIENT_COLOR_2="#c6a0f6"
GRADIENT_COLOR_3="#b7bdf8"
...
### 📘 Obsidian — смена темы
OBSIDIAN_THEME_NAME="Catppuccin"  # Название темы, как она указана в Obsidian
...
if [ -f "$VSCODE_SETTINGS" ]; then
    sed -i 's/"workbench.colorTheme": *".*"/"workbench.colorTheme": "Catppuccin Macchiato"/' "$VSCODE_SETTINGS"
else
...
### 📊 Waybar
WAYBAR_CONFIG="$HOME/.config/waybar/style/Catppuchin.css"
cp "$WAYBAR_CONFIG" "$HOME/.config/waybar/style.css"
killall waybar && waybar & disown
sleep 1

### 🧠 Rofi
ROFI_THEME="$HOME/.config/rofi/themes/CattpuchinKool-4.rasi"
...
SOURCE_CSS="$HOME/.config/firefox-themes/cattpuchin/userChrome.css"
